  ...
  return (
    <div className="rightbar">
      <div className="tab-header" onClick={() => setIsTabOpen(!isTabOpen)}>
        ...
        <button className="toggle-button">{isTabOpen ? 'Close' : 'Open'}</button>
      </div>
      <div className={`tab-content ${isTabOpen ? 'open' : 'closed'}`}>
        ...
        <div className="custom-input">
          <label>
            Give me 10 words in 
            <input type="text" placeholder="Arabic" value={language2} onChange={(e) => setLanguage2(e.target.value)} />
            using the letter 
            <input type="text" value={input2} onChange={(e) => setInput2(e.target.value)} />
          </label>
          <button onClick={() => handleSubmit(`Give me 10 words in ${language2} using the letter ${input2}`)}>Send</button>
        </div>
        <div className="custom-input">
          <label>
            Give me the word 
